...
The Worker source lives in `cloudflare/kevin-bot/index.js`. It allows production and localhost origins, adds Kevin-specific system context, and searches the `kevin-rag-index` AI Search index. Its Auto mode uses GLM-4.7-Flash with lightweight retrieval for routine questions and Qwen3-30B-A3B with enhanced retrieval for questions that need deeper analysis, while streaming progress and answer chunks over SSE.
