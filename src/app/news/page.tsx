...
const ListRow = ({ item }: { item: NewsItem }) => {
  const row = (
      <article className="group flex flex-col gap-3 py-6 border-b border-[rgba(0,0,0,0.08)] dark:border-white/20 transition-colors hover:border-foreground/70 font-normal">
        <p className="text-[12px] uppercase tracking-[0.28em] text-[rgba(0,0,0,0.6)] dark:text-foreground/70">{item.category}</p>
      <div className="flex items-start justify-between gap-3">
        <h3 className="text-[17px] leading-snug text-foreground">{item.title}</h3>
        <p className="text-[14px] text-[rgba(0,0,0,0.6)] dark:text-foreground/70 whitespace-nowrap">{formatDate(item.date)}</p>
      </div>
          ...
          {heroItem && (
            ...
            >
              ...
              <div className="absolute inset-x-0 bottom-0 bg-background/95 dark:bg-background/80 backdrop-blur-sm px-6 pb-6 pt-6 flex flex-col gap-3 border-t border-white/10 rounded-t-none">
                <p className="text-[12px] uppercase tracking-[0.28em] text-[rgba(0,0,0,0.6)] dark:text-foreground/70">
    ...
    <div className="min-h-screen bg-white text-foreground dark:bg-[#0b0b0d] dark:text-[#f5f5f5] font-medium">
      ...
      <main className="flex-1 mx-auto w-full max-w-5xl px-2 md:px-4 lg:px-0 py-6 flex flex-col gap-6">
        <section className="mt-4 space-y-2">
          <p className="text-xs uppercase tracking-[0.3em] text-[rgba(0,0,0,0.6)] dark:text-foreground/70">NEWS</p>
        ...
        <div className="flex flex-wrap gap-2 text-sm text-[rgba(0,0,0,0.6)] dark:text-foreground/70">
              ...
              onClick={() => setActiveCategory(category)}
                className={`px-3 py-1.5 rounded-full border transition-colors ${
                  activeCategory === category
                    ...
                    : "bg-[rgba(0,0,0,0.04)] border-transparent text-[rgba(0,0,0,0.6)] dark:text-foreground/70 hover:border-[rgba(0,0,0,0.08)]"
                }`}
            >
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 text-sm font-medium">
          <p className="text-[rgba(0,0,0,0.6)] dark:text-foreground/70">Showing {sortedItems.length} updates</p>
