    ...
    <div className="min-h-screen bg-white text-foreground dark:bg-[#000000] dark:text-[#f5f5f5] font-normal">
      ...
      <main className="flex-1 mx-auto w-full max-w-5xl px-2 md:px-4 lg:px-0 py-10 flex flex-col gap-16">
        <section className="mt-2 space-y-5">
          <p className="text-[12px] uppercase tracking-[0.3em] text-[rgba(0,0,0,0.6)] dark:text-foreground/70">Contact</p>
        ...
        <section className="grid gap-14 lg:grid-cols-2">
          ...
          <p className="text-[14px] uppercase tracking-[0.28em] text-[rgba(0,0,0,0.6)] dark:text-foreground/70">Direct lines</p>
            <div className="border-y border-[rgba(0,0,0,0.08)] dark:border-white/15">
              {directLines.map(({ label, value, hint, href, onClick, icon: Icon }, idx) => {
                ...
                const Row = (
                  ...
                  >
                    <div className="flex items-center gap-2 text-[12px] uppercase tracking-[0.26em] text-[rgba(0,0,0,0.6)] dark:text-foreground/70">
                      {Icon && <Icon size={14} />}
                      <span>{label}</span>
                    </div>
                    <p className="text-[17px] font-medium text-foreground">{value}</p>
                    <p className="text-[14px] text-[rgba(0,0,0,0.6)] dark:text-foreground/70">{hint}</p>
                  </div>
                );
          ...
          <div className="space-y-5">
            <p className="text-[14px] uppercase tracking-[0.28em] text-[rgba(0,0,0,0.6)] dark:text-foreground/70">Reply & scheduling</p>
            <div className="space-y-5 border border-[rgba(0,0,0,0.08)] dark:border-white/15 rounded-[18px] p-6">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <p className="text-[17px] text-black dark:text-foreground">Include who is involved and timelines for a quicker reply.</p>
                  <p className="text-[17px] text-[rgba(0,0,0,0.6)] dark:text-foreground/70">Clear context helps me reply quickly.</p>
                </div>
                <span className="px-3 py-1 rounded-full text-[11px] uppercase tracking-[0.22em] bg-[rgba(0,0,0,0.04)] text-[rgba(0,0,0,0.6)] dark:text-foreground/70">
                  <span className="text-foreground">AEDT</span>
                </span>
              </div>
              <ul className="space-y-3 text-[17px] leading-relaxed">
                {replyNotes.map((note) => (
                  <li key={note} className="flex gap-2 text-black dark:text-foreground/90">
        ...
        <section className="grid gap-14 lg:grid-cols-2">
          <div className="space-y-4">
            <p className="text-[14px] uppercase tracking-[0.28em] text-[rgba(0,0,0,0.6)] dark:text-foreground/70">Collaboration fit</p>
            <h2 className="text-[30px] font-medium text-foreground">Ways I can help</h2>
            <ul className="space-y-3 text-[17px] leading-relaxed">
              {collaborationAreas.map((area) => (
                <li key={area} className="flex gap-2 text-black dark:text-foreground/90">
                  <span className="text-[var(--accent)]">•</span>
                  <span>{area}</span>
                </li>
              ))}
          ...
          <div className="space-y-4">
            <p className="text-[14px] uppercase tracking-[0.28em] text-[rgba(0,0,0,0.6)] dark:text-foreground/70">Profiles</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {socialLinks.map(({ label, href, icon }) => (
                ...
                >
                  ...
                  <span className="text-[rgba(0,0,0,0.6)] dark:text-foreground/70">{icon}</span>
                </Link>
              ))}
            </div>
          </div>
      ...
      {showCallInfo && (
        ...
        >
          ...
          >
            ...
            <p className="text-[17px] text-[rgba(0,0,0,0.6)] dark:text-foreground/70 leading-relaxed">
